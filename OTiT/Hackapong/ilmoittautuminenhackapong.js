var main = function(){
	...
	$.ajax({
		type: 'GET',
		url: 'http://' + window.location.hostname + ':8080/hackapong/otit',
		dataType: 'json',
	...
	$.ajax({
		type: 'GET',
		url:  'http://' + window.location.hostname + ':8080/hackapong/blanko',
		dataType: 'json',
		success: function(data){
			var names = data.names;
			$.each(names, function(i){
	...
	$.ajax({
		type: 'GET',
		url:  'http://' + window.location.hostname + ':8080/hackapong/sigma',
		dataType: 'json',
		success: function(data){
			var names = data.names;
			$.each(names, function(i){
...
*/
	...
	$('.otitnappi').click(function(){
		...
		$('.sposti').val('');
		$('.btn').addClass('disabled');
		$.post( 'http://' + window.location.hostname + ':8080/hackapong/otit', JSON.stringify({"name": post, "email": email})).done(function () {
	...
	$('.blankonappi').click(function(){
		...
		$('.sposti').val('');
		$('.btn').addClass('disabled');
		$.post( 'http://' + window.location.hostname + ':8080/hackapong/blanko', JSON.stringify({"name": post, "email": email})).done(function () {
                alert("Ilmoittautuminen vastaanotettu");
                });
		});
	...
	$('.sigmanappi').click(function(){
		...
		$('.sposti').val('');
		$('.btn').addClass('disabled');
		$.post( 'http://' + window.location.hostname + ':8080/hackapong/sigma', JSON.stringify({"name": post, "email": email})).done(function () {
            alert("Ilmoittautuminen vastaanotettu");
            });
		});
